...
为了提高查询效率，SeaTable 会为存储到大数据存储中的行自动创建索引。目前，文本、数字、单选、多选、协作人、日期、创建者、创建时间、修改者、修改时间列会创建索引。目前还不支持手工创建索引。

当您在一个子表中新增或者删除一列时，对应的索引并不会立即自动创建或者删除。索引的创建和删除操作只会在下次执行“归档”操作的时候执行。
